...
bool ImGui::StrikeThroughButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 lineStart = bb.Min + style.FramePadding + ImVec2(0.0f, lineOffset);
    ImVec2 lineEnd = lineStart + ImVec2(label_size.x + lineExtension, 0.0f);
    window->DrawList->AddLine(lineStart, lineEnd, ImColor(255, 255, 255), lineThickness);
...
bool ImGui::StrikeThroughButton(const char* label, const ImVec2& size_arg)
{
    return StrikeThroughButtonEx(label, size_arg, ImGuiButtonFlags_None);
}
